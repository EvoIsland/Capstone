...
const baseConocimientoKimal = {
    proyecto: {
        nombre: 'KIMAL',
        descripcion: 'Proyecto de conectividad rural que lleva internet de alta velocidad a zonas rurales de Chile',
        objetivo: 'Reducir la brecha digital en comunidades apartadas mediante tecnología de microondas',
    ...
    cobertura: {
        regiones: [
            ...
            },
            {
                ...
                comunas: ['Andacollo', 'Combarbalá', 'Canela', 'Illapel', 'La Higuera', 'La Serena', 'Los Vilos', 'Ovalle', 'Punitaqui', 'Río Hurtado', 'Vicuña'],
                instalaciones: 11
            },
            {
                nombre: 'Atacama',
    ...
    tecnologia: {
        ...
        ventajas: ['No requiere fibra óptica', 'Instalación rápida', 'Costo-efectivo en zonas rurales', 'Alta disponibilidad'],
...
// Función para enviar consultas a Gemini Pro via REST API
async function enviarAGeminiPro(mensaje: string): Promise<string> {
    try {
        const response = await $fetch('/api/chat-rest', {
            method: 'POST',
            body: {
                mensaje: mensaje
            }
        ...
        if (response.success && (response as any).respuesta) {
            return (response as any).respuesta
        } else {
        ...
        if (error.statusCode === 429) {
            return 'Lo siento, estoy recibiendo muchas consultas en este momento. Por favor intenta en unos minutos, o contacta directamente al 600-KIMAL-1 para asistencia inmediata.'
        }
        
        if (error.statusCode === 403) {
            return 'Tengo problemas de configuración temporales. Para soporte inmediato, contacta:<br/>📞 <strong>600-KIMAL-1</strong><br/>📧 <strong>soporte@kimal.cl</strong>'
        }

        if (error.statusCode === 404) {
            return 'El servicio de IA no está disponible temporalmente. Para asistencia directa:<br/>📞 <strong>600-KIMAL-1</strong><br/>📧 <strong>soporte@kimal.cl</strong>'
        }

        // Fallback a respuesta local básica
        return generarRespuestaLocal(mensaje)
    }
}
...
function analizarIntencion(pregunta: string): string {
    ...
    // Normalizar texto para mejor análisis
    const preguntaNormalizada = preguntaLower
        .replace(/[¿?¡!.,;:]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim()
    ...
    // Análisis de intención basado en contexto
    ...
    if (palabras.some(p => ['precio', 'costo', 'cuanto', 'cuánto', 'plan', 'planes'].includes(p))) {
        return generarRespuestaPrecios(pregunta)
    }
    
    // Si menciona una región o comuna específica
...
function buscarRegionEnPregunta(pregunta: string): any {
    ...
    return baseConocimientoKimal.cobertura.regiones.find(region => {
        ...
        // Buscar por comunas
        return region.comunas.some(comuna => 
            preguntaLower.includes(comuna.toLowerCase())
        )
    })
}

function generarRespuestaRegionEspecifica(region: any, pregunta: string): string {
...
function generarRespuestaGenerica(pregunta: string): string {
    ...
    const respuestaBase = respuestasGenericas[Math.floor(Math.random() * respuestasGenericas.length)]
...
async function enviarMensaje() {
    ...
    try {
        // Por ahora usar sistema local mientras resolvemos Gemini
        // const respuesta = await enviarAGeminiPro(pregunta)
        const respuesta = generarRespuestaLocal(pregunta)
        
        // Simular tiempo de escritura natural (1-3 segundos)
...
onMounted(() => {
    // Mensaje de bienvenida optimizado para modo local
